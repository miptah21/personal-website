...
| File | Purpose |
|------|---------|
| `%USERPROFILE%\.gemini\antigravity\worktree-registry.json` | **Global registry** - tracks all worktrees across all projects |
| `.agents/skills/worktree-manager/config.json` | **Skill config** - terminal, shell, port range settings |
| `.agents/skills/worktree-manager/scripts/` | **Helper scripts** - optional, can do everything manually |
| `~/tmp/worktrees/` | **Worktree storage** - all worktrees live here |
| `.agents/worktree.json` (per-project) | **Project config** - optional custom settings |
...
### Location
`%USERPROFILE%\.gemini\antigravity\worktree-registry.json`
...
| Allocate ports | `scripts/allocate-ports.ps1 -Count 2` | Manual (see above) |
| Register worktree | `scripts/register.ps1` | Manual (see above) |
| Launch agent in terminal | `scripts/launch-agent.ps1` | Manual (see below) |
| Show status | `scripts/status.ps1` | Manual |
| Cleanup worktree | `scripts/cleanup.ps1` | Manual (see Cleanup section) |
...
**With script:**
```powershell
.\.agents\skills\worktree-manager\scripts\status.ps1
.\.agents\skills\worktree-manager\scripts\status.ps1 -Project my-project
```
...
**With script:**
```powershell
.\.agents\skills\worktree-manager\scripts\cleanup.ps1 -Project my-project -Branch feature/auth -DeleteBranch
```
...
.\.agents\skills\worktree-manager\scripts\sync.ps1
```
...
### allocate-ports.ps1
```powershell
.\.agents\skills\worktree-manager\scripts\allocate-ports.ps1 -Count <count>
...
### register.ps1
```powershell
.\.agents\skills\worktree-manager\scripts\register.ps1 -Project <project> -Branch <branch> -BranchSlug <branch-slug> -WorktreePath <worktree-path> -RepoPath <repo-path> -Ports <ports> [-Task task]
```

### launch-agent.ps1
```powershell
.\.agents\skills\worktree-manager\scripts\launch-agent.ps1 -WorktreePath <worktree-path> [-Task task]
# Opens new PowerShell window with Antigravity
```

### status.ps1
```powershell
.\.agents\skills\worktree-manager\scripts\status.ps1 [-Project <name>]
# Shows all worktrees, or filtered by project
```

### cleanup.ps1
```powershell
.\.agents\skills\worktree-manager\scripts\cleanup.ps1 -Project <project> -Branch <branch> [-DeleteBranch]
...
```powershell
.\.agents\skills\worktree-manager\scripts\sync.ps1
# Reconciles registry with actual worktrees
```

### release-ports.ps1
```powershell
.\.agents\skills\worktree-manager\scripts\release-ports.ps1 -Ports <port1>,<port2>
